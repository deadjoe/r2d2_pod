...
export class DeployWorkflow extends WorkflowEntrypoint<Env, DeployParams> {
  async run(event: WorkflowEvent<DeployParams>, step: WorkflowStep) {
    ...
    const fail = async (where: string, message: string) => {
      ...
      await notify(env, `R2D2 pod failed — ${where}: ${message}`.slice(0, 400), launcher);
    };
    ...
    await step.do("mark ready", async () => {
      ...
      await notify(env, `R2D2 is up: ${proxyUrl(pod.id)} — open it from the launcher (auto-stop ${session.ttl_hours} h)`, launcher);
    });
    ...
    await step.do("ttl expired", async () => {
      ...
      await notify(env, `R2D2 pod stopped (time limit ${session.ttl_hours} h).`, launcher);
    });
  }
}
