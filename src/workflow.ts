...
import { APP_PORT, createPod, deletePod, getPod, listGpuTypes, pickCandidates, podUptime, proxyUrl } from "./runpod";
...
export class DeployWorkflow extends WorkflowEntrypoint<Env, DeployParams> {
  async run(event: WorkflowEvent<DeployParams>, step: WorkflowStep) {
    ...
    const fail = async (where: string, message: string) => {
      ...
      await notify(env, `R2D2 pod failed — ${where}: ${message}`.slice(0, 400));
    };

    // step outputs are persisted in the Workflow's history, so secrets stay out of them:
    // the access key and progress token are read from the DO inside the steps that need them
    const session = await step.do("load session", async () => {
      const s = await sessions.get(sessionId);
      if (!s) throw new Error("session not found");
      const { access_key: _k, progress_token: _t, events: _e, ...rest } = s;
      return rest;
    });
    ...
    const pod = await step.do("create pod", { retries: { limit: 4, delay: "45 seconds", backoff: "linear" } }, async () => {
      ...
      const p = await createPod(key, {
        name: `r2d2-pod-${sessionId}`,
        imageName: session.image,
        gpuTypeIds: candidates.map((c) => c.id),
        cloudType: session.cloud,
        containerDiskInGb: Number(env.CONTAINER_DISK_GB || "20"),
        ports: [`${APP_PORT}/http`, "22/tcp"],
        env: {
          R2D2_ACCESS_KEY: secrets.access_key,
          R2D2_PROGRESS_URL: progressUrl,
          R2D2_PROGRESS_TOKEN: secrets.progress_token,
        },
      });
    ...
    for (let i = 0; i < (RUNNING_MAX_MIN * 60) / RUNNING_POLL_S && !running; i++) {
      running = await step.do(`pod running? ${i}`, async () => {
        ...
        // any callback from r2d2-start also proves the container is up
        const s = await sessions.get(sessionId);
    ...
    await step.do("boot done", async () => sessions.event(sessionId, { step: "boot", status: "done", message: "container running; checking the GPU, fetching weights" }));

    // 4. wait for r2d2-start's ready callback, or for the app to answer through the proxy
    //    with the recogniser loaded
    let ready = false;
    for (let i = 0; i < (READY_MAX_MIN * 60) / READY_POLL_S && !ready; i++) {
      ready = await step.do(`app ready? ${i}`, async () => {
        ...
        try {
          const r = await fetch(`${proxyUrl(pod.id)}/api/status`, {
            headers: { "user-agent": "r2d2-pod/0.1", cookie: `r2d2_key=${s.access_key}` }, redirect: "manual",
          });
          const body = r.ok ? ((await r.json().catch(() => ({}))) as { state?: string }) : {};
          if (body.state === "ready") {
    ...
    const after = await step.do("read outcome", async () => {
      const s = await sessions.get(sessionId);
      if (!s) return null;
      const { access_key: _k, progress_token: _t, ...rest } = s;
      return rest;
    });
    ...
    // and a moving expiry would make sleepUntil sleep again after each wake
    ...
    await step.do("mark ready", async () => {
      ...
      // the access key stays on the launcher page; a push service never sees it
      await notify(env, `R2D2 is up: ${proxyUrl(pod.id)} — open it from the launcher (auto-stop ${session.ttl_hours} h)`);
    });
    ...
    await step.do("ttl expired", async () => {
      ...
      await notify(env, `R2D2 pod stopped (time limit ${session.ttl_hours} h).`);
    });
  }
}
...
export async function notify(env: Env, text: string): Promise<void> {
  ...
  try {
    await fetch(env.NOTIFY_URL, { method: "POST", headers: { "content-type": "text/plain", title: "R2D2" }, body: text });
